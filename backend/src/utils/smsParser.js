...
// Anchored to "received ETB X" first so we never accidentally grab a
// *different* ETB figure that appears later in the same message (e.g. a
// CBE message also states "Your current balance is ETB3,312.55").
const AMOUNT_RECEIVED_RE = /received\s+ETB\s*([\d,]+(?:\.\d{1,2})?)/i;
const AMOUNT_FALLBACK_RE = /ETB\s?\.?\s?([\d,]+(?:\.\d{1,2})?)/i;
...
/**
 * Main entry point. `hintProvider` lets the bridge app/device tag which
 * SIM or automation a message came from (recommended in MacroDroid — much
 * more reliable than guessing from text alone). Falls back to
 * auto-detection when not given.
 */
